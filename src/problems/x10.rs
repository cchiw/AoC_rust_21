  ...
  for line in reader.lines(){
    ...
    let mut current_total = 0;
  	for item in line.unwrap().chars() {
  		if is_open.contains(&item) {stack1.push(item);}
  		else {
  			match stack1.peek() {
					None => {current_total=*p1_points.get(&item).unwrap();break;},
					Some(last_item) => 
						if &last_item == get_open.get(&item).unwrap() {stack1.pop();}
						else {current_total=*p1_points.get(&item).unwrap();break;}
    		};
    	}
   	}
   	if current_total>0 {part1_total +=current_total;}
   	else {
