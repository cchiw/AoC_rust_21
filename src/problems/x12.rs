...
pub fn mk_edges(edges:&mut HashMap<String, HashSet<String>>){
    ...
    for line in reader.lines() {
        ...
        let group = re.captures(&content).unwrap();
				edges.entry(group[2].to_string()).or_insert_with(HashSet::new).insert(group[1].to_string());				 			
 				edges.entry(group[1].to_string()).or_insert_with(HashSet::new).insert(group[2].to_string());
	  }
}
    ...
    mk_edges(&mut edges);	   
	  ...
	  while !all_paths.is_empty(){
	  	...
	  	for (current_paths, current_used_special) in &all_paths{
	  		if done.contains(current_paths) {continue;}
				else {done.insert((current_paths).to_vec());}	  		
	  		   ...
	  		   match edges.get(tail){
	  		   	...
	  		   	Some(connections) => {
	  		   			for next in connections{
	  		   				...
	  		   				if next == &"start".to_string() {continue;}  		   				
	  		   				else if next == &end {	  		   
	  		   					total+=1;}
	  		   				else if first_letter.is_lowercase() && current_paths.contains(next) && *current_used_special==true {continue;}
	  		   				else {
	  		   				  if first_letter.is_lowercase() && current_paths.contains(next){new_used_special = true;}	  
